...
class HeadModel(BasicModel):
    ...
    def predict(self, brand: str, flow: int, flow_set: int, humid_after_cut_sum: int,
        ...
        :param humid_after_cut_sum: 切丝后出口水分
        ...
        # self.calc_work_dry(flow=flow, after_cut_humid=humid_after_cut_sum, input_humid=humid_sum, output_humid=output_humid)

        if self.timer >= self.range_1_lag:
            # if recent_humid is not None:
            #     if self.timer >= self.range_1_lag + 120:
            #         last_temp_1 = standard_temp_1 + 2 * (humid_after_cut_sum - recent_humid)
            #     else:
            #         last_temp_1 = standard_temp_1 + 2 * (humid_after_cut_sum - recent_humid)
            ##################################
            #### 使用脱水量来继续计算
            ...
            ##################################
                ...
                if self.timer >= self.range_1_lag + 120:
                    ...
                    last_temp_1 = float(self.stable_per_brand[brand][0] + self.ratio[brand][
                        0] * humid_after_cut_sum * 1.1 + standard_temp_1)
        ...
        if self.timer >= self.range_2_lag:
            ...
            #     else:
            #         last_temp_2 = standard_temp_2 + 2 * (humid_after_cut_sum - recent_humid)
            ...
            ##################################
                ...
                if self.timer >= self.range_2_lag + 120:
                    ...
                    last_temp_2 = float(self.stable_per_brand[brand][1] + self.ratio[brand][
                        1] * humid_after_cut_sum * 1.1 + standard_temp_2)
